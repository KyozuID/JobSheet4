...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (empty($id) || empty($nama) || empty($jenis) || empty($harga) || empty($produksi) || empty($kadaluarsa)) {
        ...
        // Update operation
        $sql = "UPDATE obat SET nama_obat = ?, jenis_obat = ?, harga = ?, tanggal_produksi = ?, tanggal_kadaluarsa = ? WHERE id = ?";
        $stmt = $connection->prepare($sql);

        if ($stmt) {
            $stmt->bind_param("ssdssi", $nama, $jenis, $harga, $produksi, $kadaluarsa, $id);
            $stmt->execute();
